        ...
        internal async Task UpdateAsync(Version v, Settings s)
        {
            try
            {
                ...
                if (latest > v && latest.ToString() != s.Skip)
                {
                    ...
                    {
                        var url = "https://github.com/lin-ycv/EverythingPowerToys/releases/download/installer/EverythingPTRunInstaller.exe";
                        ...
                        if (response.IsSuccessStatusCode)
                        {
                            ...
                            string installer = Path.Combine(Path.GetTempPath(), "EverythingPTRunInstaller.exe");
                            await File.WriteAllBytesAsync(installer, content);
